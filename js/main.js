...
xhr.addEventListener('load', function () {
  function renderChampions() {
    for (var key in xhr.response.data) {
      ...
      for (var i = 0; i < xhr.response.data[key].tags.length; i++) {
        $columnHalf.setAttribute('roles', 'All ' + xhr.response.data[key].tags[0] + ' ' + xhr.response.data[key].tags[1]);
      }
      ...
      $title.textContent = capitalizeFirstLetter(xhr.response.data[key].title);
      var $lore = document.createElement('p');
      $lore.textContent = xhr.response.data[key].blurb + ' ';
